...
export default async function OrderDetailPage({ params }: Props) {
  ...
  // True remaining = remaining_balance (total - down_payment) minus all recorded payments
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      {/* Header */}
      <div>
        <Link href="/orders" className="inline-flex items-center gap-1 text-sm text-stone-500 hover:text-stone-800 mb-3">
                ...
                <div key={item.id} className="flex items-start gap-3 p-3 bg-stone-50 rounded-xl">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex flex-wrap gap-x-3 gap-y-0.5 mt-1.5 text-xs text-stone-500">
                      {isSofaCategory(item.category) && item.sofa_configuration && (
                        <span>Config: <strong>{item.sofa_configuration}</strong></span>
                      )}
                    ...
                    {item.customization_note && (
                      <p className="text-xs text-stone-600 bg-amber-50 border border-amber-100 rounded px-2 py-1 mt-2 italic">
                        📝 {item.customization_note}
                      </p>
                    )}
                  </div>
        ...
        <div className="space-y-4">
            ...
            {order.customer ? (
              <div className="space-y-1 text-sm">
                ...
                {order.customer.address && (
                  <p className="text-stone-500">📍 {order.customer.address}, {order.customer.city}</p>
                )}
              </div>
            ) : (
          ...
          <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-stone-800">{formatCurrency(order.total_price)}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-stone-500">Down Payment</span>
                <span className="font-semibold text-blue-700">{formatCurrency(order.down_payment)}</span>
              </div>
              {paidSum > 0 && (
                <div className="flex justify-between">
                  <span className="text-stone-500">Additional Payments</span>
                  <span className="font-semibold text-blue-600">{formatCurrency(paidSum)}</span>
                </div>
              )}
              <div className="flex justify-between pt-2 border-t border-stone-100">
                <span className="text-stone-500 font-medium">Remaining Balance</span>
                <span className={`font-bold ${trueRemaining > 0 ? 'text-orange-600' : 'text-green-600'}`}>
              ...
              {order.payment_method && (
                <div className="flex justify-between pt-1">
                  ...
                  <span className="text-stone-600 text-xs">{order.payment_method}</span>
                </div>
              )}
            </div>
          </div>
              ...
              <div className="space-y-1 text-sm">
                {order.expected_delivery_date && (
                  <div className="flex justify-between">
                    ...
                    <span className="text-stone-700">{formatDate(order.expected_delivery_date)}</span>
                  </div>
                )}
          ...
          {(order.factory_notes || order.internal_notes) && (
            <div className="bg-white rounded-2xl border border-stone-100 shadow-sm p-5">
              ...
              {order.factory_notes && (
                <div className="mb-2">
                  <p className="text-xs text-stone-400 mb-1">Factory Notes</p>
              ...
              {order.internal_notes && (
                <div>
                  ...
                  <p className="text-sm text-stone-500 italic">{order.internal_notes}</p>
                </div>
              )}
            </div>
          )}
