...
export async function generateShowroomPDF(orderId: string): Promise<{ base64?: string; filename?: string; error?: string }> {
  ...
  const { ShowroomOrderPDF } = await import('@/components/pdf/ShowroomOrderPDF')
  ...
  const buffer = await renderToBuffer(createElement(ShowroomOrderPDF, { order: orderForPDF }) as any)
  const filename = `Showroom-Order-${order.order_number}.pdf`
